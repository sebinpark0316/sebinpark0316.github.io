function classification_similarity()
    %===============================
    % 사용자 설정
    %===============================
    boardingImgPath = 'C:\Users\ch082\OneDrive\바탕 화면\종설프3\ktx_raw\image_1.png';
    exitingImgPath  = 'C:\Users\ch082\OneDrive\바탕 화면\종설프3\ktx_raw\image_3.png';
    threshold = 0.7;
    ...
    load('trainedResnet50_BagClassifier_640.mat', 'trainedNet');  % 가방 분류기
    ...
    hasBag_boarding = classify(trainedNet, imgBoardingTrain);
    hasBag_exiting  = classify(trainedNet, imgExitingTrain);

    %===============================
    % Step 4: 결과 출력 및 시각화
    %===============================
    if isSamePerson && hasBag_boarding == "true" && hasBag_exiting == "false"
        decisionMsg = "⚠️ [경고] 가방 분실 의심 상황입니다!";
    else
        decisionMsg = "✅ 이상 없음";
    end
    ...
    subplot(1,2,1);
    imshow(imgBoarding_original);
    title(sprintf('탑승 이미지\n %s', string(hasBag_boarding)));

    subplot(1,2,2);
    imshow(imgExiting_original);
    title(sprintf('하차 이미지\n %s', string(hasBag_exiting)));
