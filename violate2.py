...
VEHICLES_TO_CONTROL = [
    ...
    },
    {
        ...
        "timed_behaviors": [
            ...
            { "time": 7.5, "new_velocity": Gf.Vec3f(-3.0, -10.0, 0.0) },
            { "time": 9.7, "new_velocity": Gf.Vec3f(-0.6, -9.5, 0.0) }
        ]
    },
    {
        "name": "Vehicle_4",
        ...
        "timed_behaviors": [
            { "time": 3.0, "new_velocity": Gf.Vec3f(0.0, 0.0, 0.0) },
            { "time": 7.5, "new_velocity": Gf.Vec3f(1.0, 10.0, 0.0) },
            { "time": 10.9, "new_velocity": Gf.Vec3f(3.3, 14.0, 0.0) }
        ]
    },
    {
        "name": "bluecar_1",
    ...
    },
    {
        ...
        "timed_behaviors": [
            { "time": 3.0, "new_velocity": Gf.Vec3f(0.0, 0.0, 0.0) },
            { "time": 7.5, "new_velocity": Gf.Vec3f(-3.2, -13.2, 0.0) },
            { "time": 9.6, "new_velocity": Gf.Vec3f(-0.9, -13.0, 0.0) }
        ]
    },
    {
        "name": "sedan",
    ...
    },
    {
        ...
        "timed_behaviors":[
            { "time": 3.0, "new_velocity": Gf.Vec3f(0.0, 0.0, 0.0) },
            { "time": 7.5, "new_velocity": Gf.Vec3f(1.5, 15.0, 0.0) },
            { "time": 10.2, "new_velocity": Gf.Vec3f(3.4, 14.0, 0.0) }
        ]
    },
    # --- ▼▼▼ Violator Vehicles ▼▼▼ ---
    {
...
class IntersectionController:
    ...
    def _handle_initial_blinking(self, dt):
        ...
        duration = 4.5    # Last for 5 seconds
    ...
    def _control_vehicles(self, dt):
        for vehicle in self.vehicles:
            ...
            # --- ▼▼▼ START: ADDED LOGIC FOR VIOLATOR VEHICLE ▼▼▼ ---
            # Get the vehicle's behavior type
            behavior = vehicle_info.get("behavior", "obey") # Default to "obey" if not specified

            # If the vehicle's behavior is 'violate'
            if behavior == "violate":
                # Ignore traffic signals and just keep driving at its defined speed
                target_velocity = vehicle_info["drive_velocity"]
                vehicle["rigid_body"].GetVelocityAttr().Set(target_velocity)
                # Use 'continue' to skip the rest of the traffic light logic for this vehicle
                continue 
            # --- ▲▲▲ END: ADDED LOGIC FOR VIOLATOR VEHICLE ▲▲▲ ---

            # The existing logic below will now only run for 'obey' vehicles
